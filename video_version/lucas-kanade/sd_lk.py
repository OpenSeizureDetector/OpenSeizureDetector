...
class sd_lk:
    ...
    Feature_Search_Period = 30 # Seconds (period between re-acquiring features to track)
    ...
    def initFeatures(self,grey):
        ...
        # If necessary, create a vector of features to populate.
        if (self.features == None):
            print "Initialising self.features[]"
            self.features = []
            for featNo in range(self.MAX_COUNT):
                self.features.append((0,0))

        newFeatures = self.getFeatures(grey)
        for newFeatNo in range(len(newFeatures)):
            print "%d - (%f,%f)" % (newFeatNo,
                                    newFeatures[newFeatNo][0],
                                    newFeatures[newFeatNo][1])
            self.features[newFeatNo] = newFeatures[newFeatNo]
